...
function applyBlockGravity() {
  ...
  for (const idx of ordered) {
    ...
    if (below < 0 || below >= size * size) continue;
    if (cells[below].classList.contains('wall')) continue;
    if (blocks.has(below)) continue;
...
setInterval(() => {
  ...
  const anySegmentSupported = worm.some(seg => {
    const belowSeg = seg + size;
    if (belowSeg < 0 || belowSeg >= size * size) return true;
    if (cells[belowSeg].classList.contains('wall')) return true;
    if (blocks.has(belowSeg)) return true;
    return false;
  });
  if (anySegmentSupported) return;
